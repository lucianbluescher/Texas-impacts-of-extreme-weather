...
![](images/clipboard-747670850.png)
...
```{r}
# Enhanced comparison maps with color ramp, scale bar, and compass
tmap_mode("plot")

night_palette <- hcl.colors(18, "Inferno", rev = TRUE)

make_viirs_map <- function(raster_obj, title_text) {
  tm_shape(raster_obj) +
    tm_raster(
...
tmap_arrange(
  make_viirs_map(feb_7_houston, "Before Storm (Feb 7, 2021)"),
  make_viirs_map(feb_16_houston, "After Storm (Feb 16, 2021)"),
  ncol = 2
)
```
...
tm_shape(blackout_sf, name = "Blackout Areas") +
  tm_polygons(col = "red", fill_alpha = 0.5) +
